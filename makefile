...
LDFLAGS=-lssl -lcrypto -ldl -lpthread -Ljson-c/lib -lcjson -lcjson_utils -Wl,--wrap,malloc -Wl,--wrap,free
CFLAGS=-O3 -Wall -Wmissing-prototypes
MACRO=-DWITH_OPENSSL -DWITH_DOM
OBJECTS=C/src/manage/manage.o C/src/device/camera_pool.o C/src/discovery/discovery.o C/src/device/device.o C/src/ptz/ptz.o C/src/snap/media.o C/src/utils/soap.o C/src/utils/bits.o C/src/network/network.o onvif/plugins/wsseapi.o onvif/plugins/wsaapi.o onvif/plugins/threads.o onvif/plugins/smdevp.o onvif/plugins/mecevp.o onvif/dom.o onvif/custom/struct_timeval.o onvif/soapC.o onvif/soapClient.o onvif/stdsoap2.o C/src/memory/memory.o
TARGET=libonvif.so
AR=$(ARCH)-ar
TAG=$(ARCH)-$(VERSION)
...
$(OBJECTS):%.o:%.c
	$(CC) $(MACRO) $(CFLAGS) $(INC_FLAGS) -c $< -fPIC -o $@
